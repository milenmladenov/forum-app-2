...
public class HomePageController {
   ...
   @GetMapping("/")
   public String homepageSetup(Model model, HttpServletRequest request, @Param("content") String content) {
      ...
      List<Post> listPosts = postService.listAll(content);
   ...
   public String allPostsList(Model model, HttpServletRequest request) throws Exception {
   ...
   public String dashboardEmptyTitleAndLinkHandler(Model model, HttpServletRequest request) throws Exception {
      setupDashboardPage(model, request);
      model.addAttribute("notice", "За да създадете публикация, всички полета трябва да бъдат попълнени!");
      return "dashboard";
   }
   ...
   public String singlePostEmptyCommentHandler(@PathVariable int id, Model model, HttpServletRequest request) {
      ...
      model.addAttribute("notice", "За да добавите коментар, трябва да въведете коментара в текстовото поле за коментар!");
      return "single-post";
   }
